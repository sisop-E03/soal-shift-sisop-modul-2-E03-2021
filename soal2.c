        ...
        if(dir != NULL){
            while((infol = readdir(dir)) != NULL){
                if(infol->d_type == DT_REG){
                    ...
                    while (s1=strtok_r(hapus, "_", &hapus)) {//misahin yg ngandung 2 nama
                        ...
                        strcat(base, hewan);
                        char *mkFolder[] = {"mkdir", "-p", base, NULL};
                        jalan("/bin/mkdir", mkFolder);
                        ...
                        strcat(base2, temp2);
                        char *masukfolder[] = {"cp", base2, base, NULL};
